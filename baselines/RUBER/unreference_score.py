...
class RUBER_unrefer(nn.Module):
    ...
    def __init__(self, query_inpt_size, reply_inpt_size,
                 embed_size, hidden_size, src_embed=None, tgt_embed=None,
                 dropout=0.5, num_layers=1, drop=True, pretrain=False, **args):
        super(RUBER_unrefer, self).__init__()


        if src_embed is not None:
            self.query_word_embed = nn.Embedding(query_inpt_size, embed_size, _weight=torch.Tensor(src_embed))
        else:
            self.query_word_embed = nn.Embedding(query_inpt_size, embed_size)

        if tgt_embed is not None:
            self.reply_word_embed = nn.Embedding(reply_inpt_size, embed_size, _weight=torch.Tensor(tgt_embed))
        else:
            self.reply_word_embed = nn.Embedding(query_inpt_size, embed_size)


        self.query_rnn = nn.GRU(embed_size, hidden_size, num_layers, bidirectional=True)
        self.reply_rnn = nn.GRU(embed_size, hidden_size, num_layers, bidirectional=True)
    ...
    def forward(self, query, in_lengths, reply, out_lengths):
        ...
        hidden_size = qh.shape[2]
        qh = qh.reshape(self.num_layers, -1, batch_size, hidden_size)[-1,:,:,:]
        qh = qh[:1, :, :] + qh[1:, :, :]    # [1, B, H]
        # qh = qh.squeeze(0)    # [B, H]
        ...
        hidden_size = rh.shape[2]
        rh = rh.reshape(self.num_layers, -1, batch_size, hidden_size)[-1,:,:,:]
        rh = rh[:1, :, :] + rh[1:, :, :]    # [1, B, H]
        # rh = rh.squeeze(0)    # [B, H]
        ...
        # print (qh.shape, rh.shape, self.M.shape)
